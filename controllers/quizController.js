...
export const createQuiz = asyncHandler(async (req, res) => {
  // 1. Change destructuring to include 'sets'
  const { title, category, sets, duration } = req.body;

  if (!title || !category || !sets?.length) {
    res.status(400);
    throw new Error("Missing required fields (Title, Category, and at least one Set)");
  }

  // 2. Validate questions inside each set
  sets.forEach(set => {
    if (!set.questions || !set.questions.length) {
        throw new Error(`Set "${set.setName}" must have at least one question`);
    }
...
export const updateQuiz = asyncHandler(async (req, res) => {
  ...
  if (Array.isArray(sets)) {
    ...
    sets.forEach(set => set.questions.forEach(validateQuestion));
    quiz.sets = sets;
  }
