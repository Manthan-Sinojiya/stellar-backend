...
//     amount: order.amount,
//     currency: order.currency,
//     key: process.env.RAZORPAY_KEY_ID, // Sending the key to the frontend dynamically
//   });
// });
...
// @desc    Get current user profile (read-only)
export const getProfile = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id).select("-password");
  res.json({ success: true, user });
});

// @desc    Update editable user profile fields and mark Step 1 partially complete
export const updateProfile = asyncHandler(async (req, res) => {
  const { fullName, fatherName, gender, dob, pincode, city, address1, address2 } = req.body;
  const user = await User.findById(req.user.id);
...
// @desc    Finalize Step 1 by validating mandatory parent details
export const completeProfileStep = asyncHandler(async (req, res) => {
  const { fatherMobile, fatherHighestEducation, fatherOccupation, fatherIncome, motherName, motherEducation, motherOccupation, extracurriculars } = req.body;

  const isValidString = (val) => val && String(val).trim().length > 0;

  // 1. Mandatory field validation
  if (!isValidString(fatherMobile) || !isValidString(fatherHighestEducation) || !isValidString(fatherOccupation) || 
      !isValidString(fatherIncome) || !isValidString(motherName) || !isValidString(motherEducation) || !isValidString(motherOccupation)) {
    res.status(400);
    throw new Error("All parent details are compulsory.");
  }

  // 2. Validate mobile (10 digits) and income (positive number)
  const mobileRegex = /^[0-9]{10}$/;
  if (!mobileRegex.test(fatherMobile)) {
    res.status(400);
    throw new Error("Father's mobile number must be a valid 10-digit number.");
  ...
  res.json({ success: true, message: "Profile validated and saved successfully" });
});

/**
 * ==========================================
 * STEP 2: EDUCATION & FILE UPLOADS
 * ==========================================
 */

// @desc    Generate a pre-signed S3 URL for secure frontend file uploads
export const uploadEducationFile = async (req, res) => {
  const { fileName, fileType, folder } = req.body;
  if (!fileName || !fileType) return res.status(400).json({ message: "Invalid file" });

  const { uploadUrl, fileUrl } = await getSignedUploadUrl({ fileName, fileType, folder });
  res.json({ uploadUrl, fileUrl });
};

// @desc    Save 10th/12th education marks and mark Step 2 complete
export const saveEducation = asyncHandler(async (req, res) => {
  const { level, percentage, cgpa, marksheetUrl } = req.body;
  ...
  if (percentage && (percentage < 0 || percentage > 100)) throw new Error("Invalid Percentage");
  if (cgpa && (cgpa < 0 || cgpa > 10)) throw new Error("Invalid CGPA");

  // Upsert education record (Update if exists, else create)
  const record = await Education.findOneAndUpdate(
    { userId: req.user.id, level },
    { userId: req.user.id, level, percentage: percentage || null, cgpa: cgpa || null, marksheetUrl },
    { upsert: true, new: true }
  );
...
// @desc    Fetch all education records for current user
export const getEducation = async (req, res) => {
  const data = await Education.find({ userId: req.user.id });
  res.json({ educations: data });
};
...
// @desc    Create a new Razorpay Order (fixed at 2000 INR)
export const createOrder = asyncHandler(async (req, res) => {
  const options = {
...
// @desc    Verify payment signature and unlock subsequent steps
export const verifyPayment = asyncHandler(async (req, res) => {
  const { razorpay_order_id, razorpay_payment_id, razorpay_signature } = req.body;

  // 1. Generate HMAC signature to verify authenticity
  const body = razorpay_order_id + "|" + razorpay_payment_id;
  const expectedSignature = crypto
    .createHmac("sha256", process.env.RAZORPAY_KEY_SECRET)
    .update(body.toString())
  ...
  // 2. Log transaction and update application progress
  const user = await User.findById(req.user.id);
  const paymentRecord = await Payment.create({
    userId: user._id,
    fullName: user.fullName,
  ...
  res.json({ success: true, message: "Payment verified and record saved." });
});

/**
 * ==========================================
 * STEP 3 & 4: APTITUDE & CERTIFICATIONS
 * ==========================================
 */

// @desc    Mark Aptitude Test as completed
export const completeAptitudeStep = asyncHandler(async (req, res) => {
  await ApplicationProgress.findOneAndUpdate(
    { userId: req.user.id },
    { step3Completed: true },
  ...
  res.json({ success: true, message: "Aptitude step completed" });
});

// @desc    Add a new certification record
export const addCertification = asyncHandler(async (req, res) => {
...
/**
 * ==========================================
 * STEP 5: INTERVIEW & PDF GENERATION
 * ==========================================
 */

// @desc    Schedule interview and generate a consolidated PDF application
export const scheduleInterview = asyncHandler(async (req, res) => {
  const { interviewDate } = req.body;
  const userId = req.user.id;

  // 1. Gather all data points for the PDF
  const user = await User.findById(userId).lean();
  const educations = await Education.find({ userId }).lean();
  const certifications = await Certification.find({ userId }).lean();

  const fullData = { ...user, interviewDate, educations, certifications };

  // 2. Generate PDF and stream it to MongoDB GridFS
  const pdfBuffer = await generateApplicationPDF(fullData);
  const bucket = req.app.locals.gridFSBucket;

  // Cleanup old PDF if it exists
  let application = await Application.findOne({ userId });
  if (application?.pdfFileId) {
    try { await bucket.delete(application.pdfFileId); } catch (e) {}
  }

  const uploadStream = bucket.openUploadStream(`app_${userId}.pdf`, { contentType: "application/pdf" });
  uploadStream.end(pdfBuffer);

  // 3. Finalize record once stream is finished
  uploadStream.on("finish", async () => {
    if (!application) {
...
// @desc    Stream the generated PDF from GridFS to the client
export const getApplicationPDF = asyncHandler(async (req, res) => {
  const application = await Application.findById(req.params.id);
...
// @desc    Get the current progress (which steps are done)
export const getProgress = asyncHandler(async (req, res) => {
  const progress = await ApplicationProgress.findOne({ userId: req.user.id }) || {
    step1Completed: false, step2Completed: false, step3Completed: false, step4Completed: false, step5Completed: false,
  };
  res.json({ success: true, progress });
});